        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "ritual_id": {"type": "string", "description": "Bridging ritual id (for complete_ritual)."},
                    "name": {"type": "string", "description": "Client name (create_client), stakeholder name (add_stakeholder), or ritual name (add_ritual)."},
                    "sponsor": {"type": "string", "description": "Client exec sponsor / bridge."},
                    "sector": {"type": "string", "description": "Client sector (e.g. 'pharma'); drives guardrails."},
                    "model_policy": {"type": "string", "description": "Per-client model policy for sensitivity routing (default 'local-sensitive')."},
                    "status": {"type": "string", "description": "Client status: active / paused / closed."},
                    "notes": {"type": "string", "description": "Freeform client notes."},
                    "role": {"type": "string", "description": "Stakeholder role/title."},
                    "archetype": {"type": "string",
                                  "enum": ["Responsiveness Gatekeeper", "High-Capacity Executor", "Authority Expert"],
                                  "description": "ITM archetype (a starting hypothesis, confirm against listening data)."},
                    "transition_stage": {"type": "string", "enum": ["stuck", "moving", "landing", "landed", "unknown"],
                                         "description": "Current transition stage."},
                    "current_identity": {"type": "string", "description": "ITM: how the stakeholder currently derives value/indispensability."},
                    "target_identity": {"type": "string", "description": "ITM: the more-valuable identity to land after the transition."},
                    "expected_loss": {"type": "string", "description": "ITM: the precise psychological loss in the transition."},
                    "target_gain": {"type": "string", "description": "ITM: what they gain that they actually want."},
                    "bridging_rituals": {"type": "string", "description": "ITM: concrete identity-affirming moments to bridge the gap."},
                    "open_loss_flag": {"type": "boolean", "description": "ITM: loss diagnosis still pending peer review."},
                    "operator": {"type": "string", "description": "The CM operator running the engagement."},
                    "assessment_note": {"type": "string", "description": "ITM: operator's assessment note."},
                    "next_ritual_due": {"type": "string", "description": "ITM: ISO datetime the next bridging ritual is due."},
                    "cycle": {"type": "string", "description": "TRI cycle label (e.g. 'Day 0', 'Day 30', 'Day 60', 'Day 90')."},
                    "total": {"type": "integer", "description": "TRI total (10-40). If 'stage' is omitted it is derived from this."},
                    "stage": {"type": "string", "description": "TRI stage; defaults from 'total' if omitted."},
                    "direction": {"type": "string", "enum": ["up", "flat", "down"], "description": "TRI direction of travel."},
                    "tri_version": {"type": "string", "description": "TRI instrument version (e.g. 'v1.2'); required for cross-cycle comparability."},
                    "priority_action": {"type": "string", "description": "TRI: priority action before the next cycle."},
